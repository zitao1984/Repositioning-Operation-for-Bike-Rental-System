function [upper,lower,opt_val] = two_region_dock(alpha,p,s,hold,T,mu,N,C)

% alpha = [0.87,0.13;0.72,0.28];
% p = [15.34,12.58;15.05,9.6];
% s = [13.81,11.33;13.55,8.63]; 
% hold = [5;5]; % holding cost
        ...
        for dec=1:numel(re)
            for i=1:numel(d{1,t})
                for j=1:numel(d{2,t})
                    ...
                    hold_cost = hold(1)*max(0,x_next-C)+hold(2)*max(N-x_next-C,0);
                
                    E_J(dec) = E_J(dec) + p_bar(1,t)*(d{1,t}(i)-w1) + p_bar(2,t)*(d{2,t}(j)-w2) + ...
                        hold_cost + value(x_next+1,t+1);
                end
            end
        end
